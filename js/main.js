const initApp = async () => {
  ...
  // add listeners for likes/dislikes
  listenForLikes()
}
...
const getDataFromDB = async () => {
  const fakeDataStream = await fetch('https://fakerapi.it/api/v1/persons?_quantity=12')
...
const renderContacts = (contacts) => {
  ...
  contacts.forEach(contact => {
    ...
    // attach each contact to a card
    const card = createPersonCard(elemObj, contact)

    // push each card to the card array
